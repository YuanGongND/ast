...
source /data/sls/scratch/yuangong/sslast/venv-ssl/bin/activate
export TORCH_HOME=/data/sls/scratch/yuangong/sslast/model/
...
tstride=10
batch_size=12
exp_dir=./exp/test-${set}-f$fstride-t$tstride-p$astpretrain-b$batch_size-lr${lr}-1
#if [ -d $exp_dir ]; then
#  echo 'exp exist'
#  exit
#fi
...
--label-csv ./class_labels_indices.csv --n_class 527 \
--lr $lr --n-epochs ${epoch} --batch-size $batch_size --save_model True \
--freqm $freqm --timem $timem --mixup ${mixup} --bal ${bal} \
--tstride $tstride --fstride $fstride --astpretrain $astpretrain